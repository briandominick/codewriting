= Deriving Static Content Dynamically from Small Data
:page-layout: post
...
Really just as a demonstration of the power of an AJYL technology stack--employing AsciiDoc, JAMstack/Jekyll, YAML, and Liquid--my professional resumé is generated along with link:https://www.codewriting.org[codewriting.org] and the _Codewriting_ book when performing a complete build of the repo that generates this very site.
...
The content doesn't matter; just note that it's largely the same, with some variance between the HTML and PDF versions.
...
Most of the specific content items are stored as _small data_ objects arranged in a link:https://idratherbewriting.com/documentation-theme-jekyll/mydoc_yaml_tutorial.html[YAML] file, with parameter values containing AsciiDoc markup.
...
YAML markup is simpler and easier to use than both, however, since they are designed for machines to read.
...
This data gets parsed with a link:https://shopify.github.io/liquid/[Liquid template] written to intertwine the AsciiDoc-formatted strings from the data file with more AssciiDoc markup, including some of the AsciiDoc-formatted content featured between the various lists that make up my resumé.
...
Both of these processes invoke the Asciidoctor rendering engine, as detailed below in <<the-build>>.
...
While a resumé is merely a novel implementation of this complex and powerful technology stack, imagine what this kind of flexibility can do for conveying complex product data in numerous permutations of editions and output formats.
