...
More likely, notes come in through a task-management system or a file-management system -- often both.
...
include::../includes/resource-types-list.adoc[tags=references]
...
There's nothing gorgeous about IDE configurations, but engineers working in a common language against a common codebase and build infrastructure should be sharing their IDE configs.
...
This is your chance to tutor Git and markup newbies and help them get comfortable with version control and flat-file scripting, however simplistic.
...
[TIP]
.Docs should enable verification of currency
